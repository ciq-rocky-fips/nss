...
class Pkcs11MlDsaTestBase : public Pk11SignatureTest {
 ...
 protected:
  Pkcs11MlDsaTestBase(void)
      ...
      params_.data = NULL; params_.len = 0; params_valid_= false;}
  ...
  const char *get_param_set(SECKEYPublicKey *key) const {
    ...
    const char *result = SECOID_FindOIDTagDescription(key->u.mldsa.params);
    if (result == NULL) {
      return "Unknown";
    }
    return result;
  ...
  void Verify(const MlDsaVerifyTestVector vec) {
    ...
    params_.len = 0;
    if (vec.ctx.size() != 0) {
      setParams(CKH_HEDGE_PREFERRED, DataBuffer(vec.ctx.data(),vec.ctx.size()));
    }
  ...
  void ImportAndSignAndVerify(const MlDsaTestVector vec) {
    ...
    Pk11SignatureTest::ImportPrivateKeyAndSignHashedData(
        privKeyBuffer, message, &sig, &sig2);
    Pkcs11SignatureTestParams params = {
        DataBuffer(), pubKeyBuffer, message, sig };
  ...
  void KeyGenFromSeed(const MlDsaKeyGenTestVector vec) {
    ...
    ScopedPK11GenericObject genPrivKey(
        PK11_CreateManagedGenericObject(slot.get(),
                                        pk11Template, pk11TemplateCount,
                                        PR_FALSE));
    ...
    ScopedSECKEYPrivateKey privKey(PK11_FindKeyByKeyID(slot.get(),
                                   &key_id, NULL));
...
class Pkcs11MlDsaKeyGenTest : public Pkcs11MlDsaTestBase,
...
class Pkcs11MlDsaVerifyTest : public Pkcs11MlDsaTestBase,
