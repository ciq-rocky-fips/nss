...
freebl_fips_DSA_PowerUpSelfTest(void)
{
    ...
    if (dsa_status != SECSuccess) {
        PORT_SetError(SEC_ERROR_NO_MEMORY);
        FIPSLOG_FAILED(NULL, NULL, "Generate a DSA public/private key pair");
        return (SECFailure);
    } else {
        FIPSLOG_SUCCESS(NULL, NULL, "Generate a DSA public/private key pair");
    }
    ...
    if (fips_request_failure("DSA","SIGN")){
        dsa_computed_signature[0] ^= 1;
    }
    if ((dsa_status != SECSuccess) ||
        ...
        FIPSLOG_FAILED("DSA","SIGN", "DSA Single-Round Known Answer Signature Test");
        dsa_status = SECFailure;
    } else {
        FIPSLOG_SUCCESS("DSA","SIGN", "DSA Single-Round Known Answer Signature Test");
        ...
        dsa_status = DSA_VerifyDigest(&dsa_public_key,
                                      &dsa_signature_item,
                                      &dsa_digest_item);
        if (fips_request_failure("DSA","VERIFY")){
            dsa_status = SECFailure;
        }
    }
    ...
    if (dsa_status != SECSuccess) {
        PORT_SetError(SEC_ERROR_LIBRARY_FAILURE);
        FIPSLOG_FAILED("DSA","VERIFY", "DSA Single-Round Known Answer Verification Test");
        return SECFailure;
    } else {
        FIPSLOG_SUCCESS("DSA","VERIFY", "DSA Single-Round Known Answer Verification Test");
    }
...
freebl_fips_RNG_PowerUpSelfTest(void)
{
    ...
    if (fips_request_failure("RNG","INTEG")){
    ...
    if ((rng_status != SECSuccess) ||
        ...
        PORT_SetError(SEC_ERROR_LIBRARY_FAILURE);
        FIPSLOG_FAILED("RNG","INTEG", "Verify DSAX to perform the RNG integrity check");
        return SECFailure;
    } else {
        FIPSLOG_SUCCESS("RNG","INTEG", "Verify DSAX to perform the RNG integrity check");
    }
